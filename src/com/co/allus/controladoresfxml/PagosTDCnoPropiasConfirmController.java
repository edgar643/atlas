 ...
 */
public class PagosTDCnoPropiasConfirmController implements Initializable {
    ...
    private transient Service<Void> servPagosTDCnoprop;
    ...
    public void mostrarTDCnopropiasConfirmData(final InfoDatosTdcPropia infoDatos) {
        try {
            Platform.runLater(new Runnable() {
                ...
                public void run() {
                    ...
                    try {
                        ...
                        final URL location = getClass().getResource("/com/co/allus/vistas/PagosTDCnoPropiasConfirm.fxml");
...
//            ex.printStackTrace();
    ...
    public Service<Void> continuar_Op() {
        servPagosTDCnoprop = new Service<Void>() {
            @Override
            protected Task<Void> createTask() {
                return new Task<Void>() {
                    ...
                    protected Void call() throws Exception {
                        ...
                        String respuestaPagoTdcnoprop = new String();
                        ...
                        final Runnable aFenix = new EnvioTipAutFenix(cliente.getCodemp(), "004");
                        ...
                        try {
                            gestorDoc.imprimir(System.getProperties().getProperty("user.name") + "##" + "Envio a MQ PagoTDC no propias = " + "##" + obtenerHoraActual());
                            //System.out.println(tramaPagoTDCprop.toString());
                            //respuestaPagoTdcnoprop = servicioSS.enviarRecibirServidor(AtlasConstantes.IP_MQ_PRINCIPAL, AtlasConstantes.PUERTO_COM_MQ, tramaPagoTDCnoprop.toString());
                            respuestaPagoTdcnoprop = servicioSS.enviarRecibirServidor(AtlasConstantes.IP_MQ_DMZ_PRINCIPAL, AtlasConstantes.PUERTO_COM_MQ_DMZ, tramaPagoTDCnoprop.toString());
                            // RespuestaTrasnfin = "900,015,000,1245456,12520035,PAGO EXITOSO,~";
                            gestorDoc.imprimir(System.getProperties().getProperty("user.name") + "##" + "Responde  MQ PagoTDC no propias = " + respuestaPagoTdcnoprop + "##" + obtenerHoraActual());

                        } catch (Exception ex) {
                            gestorDoc.imprimir(System.getProperties().getProperty("user.name") + "##" + "Error Principal MQ = " + ex.toString() + "##" + obtenerHoraActual());
                            try {
                                gestorDoc.imprimir(System.getProperties().getProperty("user.name") + "##" + "envio ctg MQ pagotdc no propias = " + "##" + obtenerHoraActual());
                                //respuestaPagoTdcnoprop = servicioSS.enviarRecibirServidor(AtlasConstantes.IP_MQ_CONTINGENCIA, AtlasConstantes.PUERTO_COM_MQ, tramaPagoTDCnoprop.toString());
                                respuestaPagoTdcnoprop = servicioSS.enviarRecibirServidor(AtlasConstantes.IP_MQ_DMZ_CONTINGENCIA, AtlasConstantes.PUERTO_COM_MQ_DMZ, tramaPagoTDCnoprop.toString());
                                gestorDoc.imprimir(System.getProperties().getProperty("user.name") + "##" + "Responde ctg MQ = " + respuestaPagoTdcnoprop + "##" + obtenerHoraActual());
                                ...
                                Platform.runLater(new Runnable() {
                                    @Override
                                    public void run() {
                                        new ModalMensajes("Error Conectándose al Servidor", "Error", ModalMensajes.MENSAJE_ERROR, ModalMensajes.MODAL_PAGOS_TDC_NO_PROPIA);
                                        cancel();
                                        // continuar_Op.cancel();
                                        progreso.progressProperty().unbind();
                                        progreso.setProgress(0);
                        ...
                        if ("000".equals(respuestaPagoTdcnoprop.split(",")[2])) {
                            final Date fechapago = new Date();

                            infotdcnoprop.setComprobante(StringUtilities.eliminarCerosLeft(respuestaPagoTdcnoprop.split(",")[3]));
                            infotdcnoprop.setFechaPago(formatoFechaPago.format(fechapago));

                            // if(infotdcprop.isPago_minnimo() || infotdcprop.isPago_total())
                            // {
                            infotdcnoprop.setValor_pago_ent(respuestaPagoTdcnoprop.split(",")[4].substring(0, respuestaPagoTdcnoprop.split(",")[4].length() - 2));
                            infotdcnoprop.setValor_pago_cent(respuestaPagoTdcnoprop.split(",")[4].substring(respuestaPagoTdcnoprop.split(",")[4].length() - 2, respuestaPagoTdcnoprop.split(",")[4].length()));
                            // }
                            ...
                            Platform.runLater(new Runnable() {
                                @Override
                                public void run() {
                                    final PagosTDCnoPropiasTrxFinController controller = new PagosTDCnoPropiasTrxFinController();
                                    controller.mostrarTrxPagoTDCnopropFin(InfoDatosTdcPropia.getInfoTdcProp());
                                }
                            });

                        } else {
                            final String coderror = respuestaPagoTdcnoprop.split(",")[2];
                            final String mensaje = respuestaPagoTdcnoprop.split(",")[3];

                            Platform.runLater(new Runnable() {
                                @Override
                                public void run() {
                                    new ModalMensajes(mensaje.isEmpty() ? "NO SE PUDO REALIZAR LA TRANSACCION" : mensaje.trim(), coderror, ModalMensajes.MENSAJE_ERROR, ModalMensajes.MODAL_PAGOS_TDC_NO_PROPIA);
                                    cancel();
                                    progreso.progressProperty().unbind();
                                    progreso.setProgress(0);
                                    progreso.setVisible(false);
