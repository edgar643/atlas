...
import com.co.allus.modelo.tpregeneracion.InfoTablaRegeneracion;
 ...
 */
public class TPRegeneracionController implements Initializable {
    ...
    @FXML
    private TableView<InfoTablaRegeneracion> tabla_datos;
    private Pagination pagination = new Pagination();
    int currentpageindex = 0;
    private static AtomicBoolean cancelartarea = new AtomicBoolean();
    private static ObservableList<InfoTablaRegeneracion> regdata = FXCollections.observableArrayList();
    ...
    private Service<ObservableList<InfoTablaRegeneracion>> task = new TPRegeneracionController.GetTarjetasTask();

    @Override
    public void initialize(URL url, ResourceBundle rb) {
        assert cancelar_op != null : "fx:id=\"cancelar_op\" was not injected: check your FXML file 'TPRegeneracion.fxml'.";
        assert colEstadoTarjeta != null : "fx:id=\"colEstadoTarjeta\" was not injected: check your FXML file 'TPRegeneracion.fxml'.";
        assert colNombreEmpresa != null : "fx:id=\"colNombreEmpresa\" was not injected: check your FXML file 'TPRegeneracion.fxml'.";
        assert colNumtarjeta != null : "fx:id=\"colNumtarjeta\" was not injected: check your FXML file 'TPRegeneracion.fxml'.";
        assert colTipoTarjeta != null : "fx:id=\"colTipoTarjeta\" was not injected: check your FXML file 'TPRegeneracion.fxml'.";
        assert continuar_op != null : "fx:id=\"continuar_op\" was not injected: check your FXML file 'TPRegeneracion.fxml'.";
        assert panel_tabla != null : "fx:id=\"panel_tabla\" was not injected: check your FXML file 'TPRegeneracion.fxml'.";
        assert progreso != null : "fx:id=\"progreso\" was not injected: check your FXML file 'TPRegeneracion.fxml'.";
        assert tabla_datos != null : "fx:id=\"tabla_datos\" was not injected: check your FXML file 'TPRegeneracion.fxml'.";
        this.location = url;
        this.resources = rb;

        continuar_op.setDisable(true);
        progreso.setVisible(false);
        colNumtarjeta.setCellValueFactory(new PropertyValueFactory<InfoTablaRegeneracion, String>("col_Numero"));
        colTipoTarjeta.setCellValueFactory(new PropertyValueFactory<InfoTablaRegeneracion, String>("col_TipoTarj"));
        colEstadoTarjeta.setCellValueFactory(new PropertyValueFactory<InfoTablaRegeneracion, String>("col_Bloqueo"));
        colNombreEmpresa.setCellValueFactory(new PropertyValueFactory<InfoTablaRegeneracion, String>("col_Empresa"));
    ...
    void continuar_op(ActionEvent event) {
        final InfoTablaRegeneracion selectedItem = tabla_datos.getSelectionModel().getSelectedItem();

        DatosRegeneracionConfirm datosAct = DatosRegeneracionConfirm.getDataAct();
        ...
        DatosRegeneracionConfirm.setDataAct(datosAct);
    ...
    public void mostrarRegeneracionIni() {
        Platform.runLater(new Runnable() {
            @Override
            public void run() {
                try {

                    final URL location = getClass().getResource("/com/co/allus/vistas/TPRegeneracion.fxml");
                    ...
                    final TableView<InfoTablaRegeneracion> tabla_datos = (TableView<InfoTablaRegeneracion>) root.lookup("#tabla_datos");
...
//                            System.out.println("TERMINO TAREA");
                             ...
                             */
                            final int numpag = regdata.size() % rowsPerPage() == 0 ? regdata.size() / rowsPerPage() : regdata.size() / rowsPerPage() + 1;
                            ...
                            pagination.setPageFactory(new Callback<Integer, Node>() {
                                ...
                                public Node call(final Integer pageIndex) {
                                    ...
                                    if (pageIndex > numpag) {
                                        ...
                                        int lastIndex = 0;
                                        int displace = regdata.size() % rowsPerPage();
                                        if (displace >= 0) {
                                            lastIndex = regdata.size() / rowsPerPage();
                                        } else {
                                            lastIndex = regdata.size() / rowsPerPage() - 1;
                                        }
                                        ...
                                        for (int i = page; i < page + itemsPerPage(); i++) {
                                            ...
                                            if (lastIndex == pageIndex) {
                                                final List<InfoTablaRegeneracion> subList = regdata.subList(pageIndex * rowsPerPage(), pageIndex * rowsPerPage() + displace);
                                                tabla_datos.setItems(FXCollections.observableArrayList(subList));
                                            } else {
                                                final List<InfoTablaRegeneracion> subList = regdata.subList(pageIndex * rowsPerPage(), pageIndex * rowsPerPage() + rowsPerPage());
                                                tabla_datos.setItems(FXCollections.observableArrayList(subList));
                                            }
                            ...
                            pagination.currentPageIndexProperty().addListener(new ChangeListener<Number>() {
                                ...
                                public void changed(final ObservableValue<? extends Number> observable, final Number oldValue, final Number newValue) {
                                    ...
                                    Platform.runLater(new Runnable() {
                                        @Override
                                        public void run() {
                                            tabla_datos.getItems().setAll(regdata.subList(newValue.intValue() * rowsPerPage(), ((newValue.intValue() * rowsPerPage() + rowsPerPage() <= regdata.size()) ? newValue.intValue() * rowsPerPage() + rowsPerPage() : regdata.size())));
                                        }
                                    });
                                }
                            });
...
//                            System.out.println("ERROR EN LA CONSULTA");
    ...
    public class GetTarjetasTask extends Service<ObservableList<InfoTablaRegeneracion>> {

        @Override
        protected Task<ObservableList<InfoTablaRegeneracion>> createTask() {
            return new Task() {
                @Override
                protected Object call() throws Exception {
                    ...
                    tramaListarTDC.append(",");
                    tramaListarTDC.append(AtlasConstantes.COD_REGENERACION_PREP); // dinamico
                    tramaListarTDC.append(",");
...
//                        System.out.println("contingencia");
                        ...
                        try {
                            ...
                            Platform.runLater(new Runnable() {
                                @Override
                                public void run() {
                                    if (TPRegeneracionController.cancelartarea.get()) {
//                                        System.out.println("cancelo tarea");
                    ...
                    if ("000".equals(Respuesta.split(",")[2])) {
                        ...
                        for (int i = 0; i < Ltarjetas.length; i++) {
                            ...
                            final InfoTablaRegeneracion tarjeta = new InfoTablaRegeneracion(
                                    tarjetaPIN,
                                    //datos[0].trim(),
